// --- 1. İÇE AKTARIMLAR VE BAĞIMLILIKLAR ---
import { useState, useEffect } from 'react';
import { Icons } from './components/Icons.jsx';

// --- 2. DİL ÇEVİRİ DOSYALARI (TRANSLATIONS) ---
// Ekranda 't(key)' fonksiyonu ile çağrılan tüm metinler buradan gelir.
const translations = {
    tr: {
        ...
        pastDueRents: "Kira Ödeme Takibi", markAsPaid: "Ödendi İşaretle", tenant: "Kiracı", amount: "Kira Tutarı", lateFee: "Gecikme Cezası", sortBy: "Sırala:", sortStatus: "Duruma Göre", sortDate: "Tarihe Göre", paid: "ÖDENDİ", overdue: "GECİKTİ",
        dueDate: "Son Ödeme", paymentMethod: "Ödeme Yöntemi", paymentDate: "Ödeme Tarihi", paymentTime: "Saat",
        vendorNameLabel: "Firma / Kişi Adı", vendorNamePlaceholder: "Örn: City Plumbing",
        vendorSpecialtyLabel: "Uzmanlık Alanı", vendorSpecialtyPlumbing: "Tesisat (Plumbing)", vendorSpecialtyElectrical: "Elektrik (Electrical)", vendorSpecialtyGeneral: "Genel Bakım (General)",
        vendorPhoneLabel: "İletişim Numarası", vendorPhonePlaceholder: "+1 555 000 0000", vendorSubmitBtn: "Sisteme Ekle",
        noticeToLabel: "Kime Gönderilecek?", noticeAllCommunities: "Tüm Siteler (All Communities)",
        noticeTitleLabel: "Duyuru Başlığı", noticeTitlePlaceholder: "Örn: Hafta Sonu Su Kesintisi",
        noticeMessageLabel: "Mesaj İçeriği", noticeMessagePlaceholder: "Mesajınızı buraya yazın...", noticeSubmitBtn: "Gönder"
    },
    en: {
        ...
        pastDueRents: "Rental Payment Track", markAsPaid: "Mark as Paid", tenant: "Tenant", amount: "Rent Amount", lateFee: "Late Fee", sortBy: "Sort by:", sortStatus: "Status", sortDate: "Date", paid: "PAID", overdue: "OVERDUE",
        dueDate: "Due Date", paymentMethod: "Payment Method", paymentDate: "Payment Date", paymentTime: "Time",
        vendorNameLabel: "Company / Contact Name", vendorNamePlaceholder: "e.g., City Plumbing",
        vendorSpecialtyLabel: "Specialty", vendorSpecialtyPlumbing: "Plumbing", vendorSpecialtyElectrical: "Electrical", vendorSpecialtyGeneral: "General Maintenance",
        vendorPhoneLabel: "Contact Number", vendorPhonePlaceholder: "+1 555 000 0000", vendorSubmitBtn: "Add to System",
        noticeToLabel: "Send To?", noticeAllCommunities: "All Communities",
        noticeTitleLabel: "Notice Title", noticeTitlePlaceholder: "e.g., Weekend Water Outage",
        noticeMessageLabel: "Message Content", noticeMessagePlaceholder: "Type your message here...", noticeSubmitBtn: "Send"
    }
};
...
const generateWorkOrderId = () => {
    ...
    // Ayların İngilizce kısa isimleri (3 harfli standart format)
    const monthNames = ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"];

    const today = new Date();
    const mmm = monthNames[today.getMonth()]; // O anki ayın metin halini alır (Örn: "JUN")
    const dd = String(today.getDate()).padStart(2, '0'); // Gün (Örn: "11")
    const yy = String(today.getFullYear()).slice(-2); // Yıl (Örn: "26")

    const dateStr = `${mmm}${dd}${yy}`; // Sonuç: JUN1126, AUG1526, SEP0126 vb.

    // Yeni format: WI-XXX-XXXX-MMMddyy
    return `WI-${getStr(4)}-${getStr(4)}-${dateStr}`;
};
...
// --- 3. SAHTE YAPAY ZEKA VERİTABANI (MOCK AI DB) ---need real aiSearching
...
// ============================================================================
// --- ANA UYGULAMA BİLEŞENİ (MAIN APP COMPONENT) ---
// ============================================================================
export default function App() {

    // --- STATE TANIMLAMALARI (Durum Yönetimi) ---
    ...
    const [rentSortBy, setRentSortBy] = useState('status');

    // --- API ÇAĞRILARI VE FONKSİYONLAR ---
    const loadDatabase = () => {
    ...
    const loadOverdueRents = () => {
        if (!isAuthenticated) return;
        fetch('/api/rents/all')
            .then(res => {
                if(res.ok) return res.json();
                return fetch('/api/rents/overdue').then(r => r.json());
            })
            .then(data => setOverdueRents(Array.isArray(data) ? data : []))
            .catch(err => console.error("Kira verisi yüklenemedi:", err));
    };
   ...
   useEffect(() => {
    ...
    const markRentAsPaid = async (rentId) => {
        try {
            const response = await fetch(`/api/rents/${rentId}/pay`, {
                method: 'PUT',
                headers: { 'X-XSRF-TOKEN': getCsrfToken() }
            });
            if (!response.ok) { alert("Sunucu Hatası: İşlem reddedildi."); return; }
            loadOverdueRents();
        } catch (error) {
    ...
    const handleAddCommunity = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                alert(`Sunucu Hatası (${response.status}): Yetkiniz yok veya CSRF Token eksik.`);
                return;
            }
            setNewCommunityName(''); loadDatabase();
        } catch (error) { console.error(error); alert("Bağlantı hatası oluştu."); }
    };
    ...
    const handleAddApartment = async (e) => {
        ...
        try {
            const response = await fetch(`/api/communities/${selectedCommunityId}/apartments`, {
                method: 'POST', headers: { 'Content-Type': 'application/json', 'X-XSRF-TOKEN': getCsrfToken() },
                body: JSON.stringify({ id: val })
            });
            if (!response.ok) { alert("Sunucu Hatası!"); return; }
    ...
    const deleteCommunity = async (e, commId) => {
        ...
        if (window.confirm(t('confirmDelComm'))) {
            const response = await fetch(`/api/communities/${commId}`, { method: 'DELETE', headers: { 'X-XSRF-TOKEN': getCsrfToken() } });
            if (!response.ok) { alert("Sunucu Hatası!"); return; }
    ...
    const deleteApartment = async (e, commId, aptId) => {
        ...
        if (window.confirm(t('confirmDelApt'))) {
            const response = await fetch(`/api/apartments/${aptId}`, { method: 'DELETE', headers: { 'X-XSRF-TOKEN': getCsrfToken() } });
            if (!response.ok) { alert("Sunucu Hatası!"); return; }
    ...
    const deleteItem = async (e, communityId, aptId, itemId) => {
        ...
        if (window.confirm(t('confirmDelItem'))) {
            const response = await fetch(`/api/items/${itemId}`, { method: 'DELETE', headers: { 'X-XSRF-TOKEN': getCsrfToken() } });
            if (!response.ok) { alert("Sunucu Hatası!"); return; }
            loadDatabase();
        }
    };
    ...
    const saveItem = async () => {
        ...
        try {
            const response = await fetch(url, { method: method, headers: { 'X-XSRF-TOKEN': getCsrfToken() }, body: formData });
            if (!response.ok) { alert("Sunucu Hatası! Kayıt başarısız."); return; }
   ...
   const handleLogout = () => {
    ...
    const sortedRents = [...overdueRents].sort((a, b) => {
        if (rentSortBy === 'status') {
    ...
    // --- MODAL İÇERİKLERİ (Tıklayınca Açılan Pencerelerin İçeriği) ---
    const renderModalContent = () => {
        switch(activeModal) {
            case 'sites':
                // Modal: Toplam Siteler Listesi
                return (
                    <div className="divide-y divide-slate-100 p-2">
                        {communities.length > 0 ? communities.map(c => (
                            <div key={c.id} className="p-4 flex justify-between items-center hover:bg-slate-50 rounded-xl transition-colors">
                                <div className="flex items-center gap-3">
                                    <div className="bg-blue-100 text-blue-700 p-2 rounded-lg"><Icons.Layout size={18} /></div>
                                    <span className="font-bold text-slate-800 text-lg">{c.name}</span>
                                </div>
                                <span className="text-xs bg-slate-100 text-slate-600 font-bold px-4 py-2 rounded-full border border-slate-200">{c.apartments?.length || 0} {t('units')}</span>
                            </div>
                        )) : <p className="p-10 text-center text-slate-500 font-medium">{t('noCommunity')}</p>}
                    </div>
                );
            case 'units':
                // Modal: Toplam Daireler Listesi
                const allUnits = communities.flatMap(c => c.apartments?.map(a => ({...a, commName: c.name})) || []);
                return (
                    <div className="divide-y divide-slate-100 p-2">
                        {allUnits.length > 0 ? allUnits.map((a, i) => (
                            <div key={i} className="p-4 flex justify-between items-center hover:bg-slate-50 rounded-xl transition-colors">
                                <div className="flex items-center gap-4">
                                     <div className="bg-emerald-100 text-emerald-700 p-2 rounded-lg"><Icons.Home size={18} /></div>
                                    <div>
                                        <span className="font-bold text-slate-800 text-lg">{a.id}</span>
                                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{a.commName}</p>
                                    </div>
                                </div>
                                <span className="text-xs font-bold bg-slate-100 text-slate-600 px-4 py-2 rounded-full border border-slate-200">{a.items?.length || 0} {t('workOrders')}</span>
                            </div>
                        )) : <p className="p-10 text-center text-slate-500 font-medium">Kayıtlı daire bulunmuyor.</p>}
                    </div>
                );
            case 'tasks':
                // Modal: Bekleyen İş Emirleri Listesi
                ...
                return (
                    <div className="divide-y divide-slate-100 p-2">
                        {pending.length > 0 ? pending.map((task, i) => (
                            <div key={i} className="p-4 flex justify-between items-center hover:bg-slate-50 rounded-xl transition-colors">
                                <div className="flex items-center gap-4">
                                     <div className="bg-amber-100 text-amber-700 p-2 rounded-lg"><Icons.Edit size={18} /></div>
                                    <div>
                                        <span className="font-bold text-slate-800 text-base">{task.title}</span>
                                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{task.comm} / {task.apt}</p>
                                    </div>
                                </div>
                                <span className="text-[10px] font-black text-amber-600 uppercase bg-amber-50 px-3 py-1.5 rounded-md border border-amber-200">
                                    {t(task.status.toLowerCase() === 'todo' ? 'todo' : 'inProgress')}
                                </span>
                            </div>
                        )) : <p className="p-10 text-center text-slate-500 font-medium">Bekleyen iş yok.</p>}
                    </div>
                );
            case 'vendor':
                // Modal: Tedarikçi / Görevli Ekleme Formu
                return (
                    <div className="p-8 space-y-6">
                        <div className="text-center mb-6">
                            <div className="text-5xl mb-4">👷‍♂️</div>
                            <h3 className="text-2xl font-black text-slate-800">{t('newVendor')}</h3>
                        </div>
                        <div className="space-y-4">
                            <div><label className="text-xs font-bold text-slate-500 uppercase">{t('vendorNameLabel')}</label><input className="w-full p-4 mt-2 bg-slate-50 border border-slate-200 rounded-xl outline-none" placeholder={t('vendorNamePlaceholder')} /></div>
                            <div><label className="text-xs font-bold text-slate-500 uppercase">{t('vendorSpecialtyLabel')}</label>
                                <select className="w-full p-4 mt-2 bg-slate-50 border border-slate-200 rounded-xl outline-none">
                                    <option>{t('vendorSpecialtyPlumbing')}</option><option>{t('vendorSpecialtyElectrical')}</option><option>{t('vendorSpecialtyGeneral')}</option>
                                </select>
                            </div>
                            <div><label className="text-xs font-bold text-slate-500 uppercase">{t('vendorPhoneLabel')}</label><input className="w-full p-4 mt-2 bg-slate-50 border border-slate-200 rounded-xl outline-none" placeholder={t('vendorPhonePlaceholder')} /></div>
                            <button className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl mt-4 transition-colors">{t('vendorSubmitBtn')}</button>
                        </div>
                    </div>
                );
            case 'notice':
                // Modal: Kiracılara Duyuru Gönderme Formu
                return (
                    <div className="p-8 space-y-6">
                        <div className="text-center mb-6">
                            <div className="text-5xl mb-4">📢</div>
                            <h3 className="text-2xl font-black text-slate-800">{t('sendNotice')}</h3>
                        </div>
                        <div className="space-y-4">
                            <div><label className="text-xs font-bold text-slate-500 uppercase">{t('noticeToLabel')}</label>
                                <select className="w-full p-4 mt-2 bg-slate-50 border border-slate-200 rounded-xl outline-none">
                                    <option>{t('noticeAllCommunities')}</option>
                                    {communities.map(c => <option key={c.id}>{c.name}</option>)}
                                </select>
                            </div>
                            <div><label className="text-xs font-bold text-slate-500 uppercase">{t('noticeTitleLabel')}</label><input className="w-full p-4 mt-2 bg-slate-50 border border-slate-200 rounded-xl outline-none" placeholder={t('noticeTitlePlaceholder')} /></div>
                            <div><label className="text-xs font-bold text-slate-500 uppercase">{t('noticeMessageLabel')}</label><textarea className="w-full p-4 mt-2 bg-slate-50 border border-slate-200 rounded-xl h-32 outline-none resize-none" placeholder={t('noticeMessagePlaceholder')} /></div>
                            <button className="w-full py-4 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl mt-4 transition-colors flex justify-center items-center gap-2"><span>✉️</span> {t('noticeSubmitBtn')}</button>
                        </div>
                    </div>
                );
            default: return null;
        ...
        <div className="min-h-screen flex flex-col bg-slate-50 text-slate-800 font-sans relative">
            ...
            <div className="max-w-[100rem] mx-auto w-full p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 items-start">
                ...
                <aside className="lg:col-span-3 flex flex-col gap-6 self-stretch">
                    ...
                    {/* Sol Menü: Arama ve Site Listesi Alanı */}
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col">
                        ...
                        <div className="flex flex-col gap-2 max-h-[50vh] overflow-y-auto custom-scrollbar pr-2">
                            {filteredCommunities.map(comm => (
                                <div key={comm.id} className={`w-full rounded-xl border-2 hover:bg-slate-50 transition-all group flex items-center pr-2 mb-2 ${selectedCommunityId === comm.id ? 'border-blue-400 bg-blue-50/50' : 'border-transparent'}`}>
                                    <button onClick={() => { setSelectedCommunityId(comm.id); setSelectedApartmentId(null); }} className={`flex-1 p-4 text-left font-bold text-base transition-colors ${selectedCommunityId === comm.id ? 'text-blue-800' : 'text-slate-700 group-hover:text-blue-700'}`}>{comm.name}</button>
                    ...
                    {/* Sol Menü: Mini İstatistikler ve Yardım Kutusu */}
                ...
                <main className="lg:col-span-9">
                    ...
                    {/* 3.A. GENEL PORTFÖY EKRANI (Hiçbir site seçili değilken görünen ekran) */}
                    {selectedCommunityId === null && (
                        <div className="space-y-8 animate-in">

                            {/* 3.A.1. KPI KARTLARI (Toplam Site, Daire vb.) */}
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                {[
                                    ...
                                    <div key={i} onClick={() => kpi.id !== 'expense' ? setActiveModal(kpi.id) : null} className={`bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-5 transition-all ${kpi.id !== 'expense' ? 'cursor-pointer hover:border-blue-400 hover:shadow-md transform hover:-translate-y-1' : ''}`}>
                            ...
                            {/* 3.A.2. HIZLI İŞLEMLER (Quick Actions) KUTUSU */}
                            ...
                            {/* 3.A.3. KİRA ÖDEME TAKİBİ (RENT TRACKER) MODÜLÜ */}
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-md overflow-hidden animate-in">
                                <div className="bg-slate-800 p-5 flex flex-col sm:flex-row justify-between items-center gap-4">
                                    <div className="flex items-center gap-3">
                                        <div className="text-2xl">💰</div>
                                        <h3 className="font-black text-xl text-white tracking-tight">{t('pastDueRents')}</h3>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <span className="text-xs font-bold text-slate-300 uppercase">{t('sortBy')}</span>
                                        <select
                                            className="bg-slate-700 border border-slate-600 text-white text-xs font-bold rounded-lg px-3 py-2 outline-none cursor-pointer hover:bg-slate-600 transition-colors"
                                            value={rentSortBy}
                                ...
                                {sortedRents.length > 0 ? (
                                    <div className="divide-y divide-slate-100 max-h-[400px] overflow-y-auto custom-scrollbar">
                                        {sortedRents.map(rent => (
                                            <div key={rent.id} className={`p-5 flex flex-col sm:flex-row sm:justify-between sm:items-center transition-colors gap-4 ${rent.status === 'OVERDUE' ? 'bg-rose-50/30 hover:bg-rose-50' : 'bg-white hover:bg-emerald-50/30'}`}>
                                                <div className="flex items-center gap-5 w-full sm:w-auto">
                                                    {rent.status === 'OVERDUE' ? (
                                                        <div className="w-12 h-12 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center text-xl shadow-inner shrink-0">🚨</div>
                                                    ) : (
                                                        <div className="w-12 h-12 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center text-xl shadow-inner shrink-0">✅</div>
                                                    )}
                                                    <div className="flex-1 w-full">
                                                        <p className="text-lg font-bold text-slate-800 flex items-center gap-3 flex-wrap">
                                                            {rent.tenantName || t('tenant')}
                                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 bg-slate-100 px-2 py-1 rounded-md border border-slate-200">Apt: {rent.apartment?.id || '-'}</span>
                                                            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md border ${rent.status === 'OVERDUE' ? 'bg-rose-100 text-rose-700 border-rose-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200'}`}>
                                                                {rent.status === 'OVERDUE' ? t('overdue') : t('paid')}
                                                            </span>
                                                        </p>

                                                        <p className="text-sm text-slate-600 mt-1.5 font-medium">
                                                            {t('amount')}: <span className="font-bold text-slate-800">${rent.rentAmount}</span>
                                                            {rent.lateFeeApplied && rent.status === 'OVERDUE' && (
                                                                <span className="ml-3 text-rose-600 bg-rose-50 px-2 py-0.5 rounded text-xs border border-rose-100 font-bold">
                                                                    + ${rent.lateFee} {t('lateFee')}
                                                                </span>
                                                            )}
                                                            <span className="ml-4 text-xs text-slate-400">{t('dueDate')}: {rent.dueDate ? new Date(rent.dueDate).toLocaleDateString(lang === 'tr' ? 'tr-TR' : 'en-US') : '-'}</span>
                                                        </p>

                                                        <div className="flex flex-wrap items-center gap-4 mt-3 pt-3 border-t border-slate-100/60 text-[11px] text-slate-500">
                                                            <span className="font-semibold text-slate-600">{t('paymentMethod')}: <span className="font-bold text-slate-800">{rent.paymentMethod || '-'}</span></span>
                                                            <span className="font-semibold text-slate-600">{t('paymentDate')}: <span className="font-bold text-slate-800">{rent.paymentDate ? new Date(rent.paymentDate).toLocaleDateString(lang === 'tr' ? 'tr-TR' : 'en-US') : '-'}</span></span>
                                                            <span className="font-semibold text-slate-600">{t('paymentTime')}: <span className="font-bold text-slate-800">{rent.paymentTime || '-'}</span></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-6 mt-3 sm:mt-0">
                                                    <div className="text-right">
                                                        <p className={`text-[10px] font-black uppercase tracking-widest ${rent.status === 'OVERDUE' ? 'text-rose-400' : 'text-emerald-400'}`}>Toplam Tutar</p>
                                                        <p className={`text-2xl font-black ${rent.status === 'OVERDUE' ? 'text-rose-600' : 'text-emerald-600'}`}>
                                                            ${rent.status === 'OVERDUE' ? rent.rentAmount + (rent.lateFee || 0) : rent.rentAmount}
                                                        </p>
                                                    </div>
                                                    {rent.status === 'OVERDUE' && (
                                                        <button onClick={() => markRentAsPaid(rent.id)} className="bg-emerald-500 hover:bg-emerald-600 text-white transition-all font-bold px-4 py-2.5 rounded-xl text-sm shadow-md hover:shadow-lg hover:-translate-y-0.5 flex items-center gap-2">
                                                            <span>💳</span> {t('markAsPaid')}
                                                        </button>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    <div className="p-10 flex flex-col items-center justify-center text-center bg-slate-50">
                                        <span className="text-5xl mb-4 opacity-50">✅</span>
                                        <p className="text-slate-600 font-bold text-lg mb-2">
                                            {lang === 'tr' ? 'Harika! Bekleyen veya geciken kira yok.' : 'Great! No pending or overdue rents.'}
                                        </p>
                                        <p className="text-slate-500 text-sm mb-6 max-w-md">
                                            {lang === 'tr'
                                                ? 'Sistem her ayın 1\'inde otomatik olarak kiraları oluşturacaktır. Şimdilik manuel test etmek isterseniz aşağıdaki butona basabilirsiniz.'
                                                : 'The system will automatically generate rents on the 1st of every month. To test it manually now, click the button below.'}
                                        </p>
                                        <button
                                            onClick={async () => {
                                                try {
                                                    await fetch('/api/rents/trigger-monthly', { method: 'POST', headers: {'X-XSRF-TOKEN': getCsrfToken()} });
                                                    loadOverdueRents();
                                        ...
                                        >
                                            <span>⚡</span> {lang === 'tr' ? 'Test: Kiraları Şimdi Oluştur' : 'Test: Generate Rents Now'}
                                        </button>
                                    </div>
                                )}
                            </div>

                            {/* 3.A.4. BOŞ DURUM ALANI (Hiç site eklenmemişse görünür) */}
                            ...
                            {/* 3.A.5. ACİL VE YÜKSEK ÖNCELİKLİ İŞLER LİSTESİ */}
                            {(communities || []).length > 0 && (
                    ...
                    {/* 3.B. SİTE DETAY EKRANI (Bir Site Seçildiğinde Daireleri Gösterir) */}
                    {selectedCommunityId !== null && selectedApartmentId === null && selectedCommunity && (
                        <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm space-y-8 animate-in">
                    ...
                    {/* 3.C. DAİRE DETAY EKRANI VE İŞ EMRİ FORMLARI */}
                    {selectedCommunityId !== null && selectedApartmentId !== null && selectedApartment && (
                        <div className="space-y-6 animate-in">
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                                ...
                                {/* 3.C.2. YENİ İŞ EMRİ EKLEME / DÜZENLEME FORMU */}
                                {isFormOpen && (
            ...
            {/* 4. GLOBAL AÇILIR PENCERE (MODAL OVERLAY) - (Arka planı karartıp ortada pencere açan sistem) */}
            {activeModal && (
