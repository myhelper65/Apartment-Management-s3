...
export default function App() {
   ...
   useEffect(() => {
           ...
           if (savedLang) {
               ...
               fetch('https://ipapi.co/json/')
                   ...
                   .catch(() => { setLang('en'); setIsLangChecked(true); })
                   .finally(() => setIsLangChecked(true));
           }
       }, []);
    ...
    // YENİ VE PROFESYONEL ÖNİZLEME FONKSİYONU
    const handlePreview = (url) => {
        if (!url) return;

        let safeUrl = url.includes('?key=') ? url.split('?key=')[0] + '?key=' + encodeURIComponent(url.split('?key=')[1]) : url;

        // Eğer dosya henüz sisteme yüklenmemiş "yerel (blob)" bir dosyaysa direkt aç:
    ...
    const handleAddCommunity = async (e) => {
        ...
        try {
            await fetch('/api/communities', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: val }) });
    ...
    const handleAddApartment = async (e) => {
        ...
        try {
            await fetch(`/api/communities/${selectedCommunityId}/apartments`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: val }) });
    ...
    const deleteCommunity = async (e, commId) => {
        ...
        if (window.confirm(t('confirmDelComm'))) {
            await fetch(`/api/communities/${commId}`, { method: 'DELETE' });
    ...
    const updateItemStatus = async (communityId, aptId, itemId, newStatus) => {
        await fetch(`/api/items/${itemId}/status?status=${newStatus}`, { method: 'PUT' });
        loadDatabase();
    };
   ...
   const handleLogout = () => {
    ...
    // GİRİŞ EKRANI (HTML BURADA BAŞLIYOR)
    if (!isAuthenticated || !isLangChecked) return (
        <div className="min-h-screen w-full bg-[url('https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=2000&auto=format&fit=crop')] bg-cover bg-center relative flex flex-col justify-center p-6">
            <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-lg z-0"></div>
    ...
    // YÖNETİM PANELİ (GİRİŞ YAPILMIŞ HALİ)
    return (
